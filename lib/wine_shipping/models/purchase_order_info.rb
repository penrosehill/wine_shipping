  ...
  class PurchaseOrderInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WineShipping::PurchaseOrderInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WineShipping::PurchaseOrderInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'transport_method')
        self.transport_method = attributes[:'transport_method']
      end
      ...
      if attributes.key?(:'receive_to_location')
        self.receive_to_location = attributes[:'receive_to_location']
      end
    ...
    def valid?
      transport_method_validator = EnumAttributeValidator.new('Object', [1, 2])
    ...
    def transport_method=(transport_method)
      validator = EnumAttributeValidator.new('Object', [1, 2])
