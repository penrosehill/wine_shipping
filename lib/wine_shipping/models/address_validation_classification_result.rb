...
module WineShipping
  class AddressValidationClassificationResult
    attr_accessor :transaction_reference_number

    attr_accessor :address_validation_indicator

    attr_accessor :address_classification_indicator
    ...
    def self.openapi_types
      {
        ...
        :'address_validation_indicator' => :'Object',
        :'address_classification_indicator' => :'Object',
        :'address_candidates' => :'Object',
        :'pal_details' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WineShipping::AddressValidationClassificationResult` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WineShipping::AddressValidationClassificationResult`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'address_validation_indicator')
        self.address_validation_indicator = attributes[:'address_validation_indicator']
      end

      if attributes.key?(:'address_classification_indicator')
        self.address_classification_indicator = attributes[:'address_classification_indicator']
      end

      if attributes.key?(:'address_candidates')
        if (value = attributes[:'address_candidates']).is_a?(Array)
          self.address_candidates = value
        end
      end

      if attributes.key?(:'pal_details')
        if (value = attributes[:'pal_details']).is_a?(Array)
