  ...
  class SalesOrderHeaderInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WineShipping::SalesOrderHeaderInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WineShipping::SalesOrderHeaderInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [transmission_id, order_no, pono, shipment_key, order_type, order_source, order_date, gift_card, gift_card_message, gift_note_product_id, tags, special_instructions, consumer_tax_collected, consumer_shipping_price].hash
    end
