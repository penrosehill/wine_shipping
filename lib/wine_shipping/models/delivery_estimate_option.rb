...
module WineShipping
  class DeliveryEstimateOption
    attr_accessor :delivery_date_estimate
    ...
    def self.attribute_map
      {
        :'delivery_date_estimate' => :'DeliveryDateEstimate',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WineShipping::DeliveryEstimateOption` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WineShipping::DeliveryEstimateOption`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'delivery_date_estimate')
        self.delivery_date_estimate = attributes[:'delivery_date_estimate']
      end
    ...
    def hash
      [delivery_date_estimate, shipment_date, cutoff_time, shipping_warehouse, shipping_options].hash
    end
